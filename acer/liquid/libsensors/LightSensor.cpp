      ...
      mHasPendingEvent(false)
{
    ...
    int flags = 0;
    if (!ioctl(dev_fd, LIGHT_SENSOR_IOCTL_GET_ENABLE, &flags)) {
        if (flags) {
            mEnabled = 1;
            setInitialState();
        }
...
int LightSensor::enable(int32_t, int en) {
    ...
    if (flags != mEnabled) {
        ...
        err = ioctl(dev_fd, LIGHT_SENSOR_IOCTL_ENABLE, &flags);
        err = err<0 ? -errno : 0;
        LOGE_IF(err, "LIGHT_SENSOR_IOCTL_ENABLE failed (%s)", strerror(-err));
        if (!err) {
            mEnabled = en ? 1 : 0;
            if (en) {
                setInitialState();
    ...
    while (count && mInputReader.readEvent(&event)) {
        ...
        if (type == EV_ABS) {
            if (event->code == EVENT_TYPE_LIGHT) {
                if (event->value != -1) {
			LOGD("Light(%d, %d)", event->value, indexToValue(event->value));
...
float LightSensor::indexToValue(size_t index) const
{
    static const float luxValues[10] = {
            7.0, 59.0, 137.0, 1171.0, 1559.0,
            1817.0, 2076.0, 2335.0, 2593.0, 4095.0
    };
