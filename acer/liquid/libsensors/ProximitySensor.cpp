...
ProximitySensor::ProximitySensor()
    : SensorBase("/dev/proximity", "proximity"),
      mEnabled(0),
      mInputReader(4),
      mHasPendingEvent(false)
{
    ...
    int flags = 0;
    if (!ioctl(dev_fd, PROXIMITY_SENSOR_IOCTL_GET_ENABLE, &flags)) {
        mEnabled = 1;
        if (flags) {
            setInitialState();
        }
...
int ProximitySensor::enable(int32_t, int en) {
    ...
    if (newState != mEnabled) {
        ...
        int flags = newState;
        err = ioctl(dev_fd, PROXIMITY_SENSOR_IOCTL_ENABLE, &flags);
        err = err<0 ? -errno : 0;
        LOGE_IF(err, "PROXIMITY_SENSOR_IOCTL_ENABLE failed (%s)", strerror(-err));
        if (!err) {
            mEnabled = newState;
            if (en) {
                setInitialState();
